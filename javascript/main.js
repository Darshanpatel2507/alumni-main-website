...
function renderEvents() {
  ...
  grid.innerHTML = eventsData
    .map(
      (e, i) => `
    <div class="event-card" style="animation-delay: ${i * 0.1}s" id="event-${e.id}">
      <span class="event-badge ${e.type === "person" ? "badge-person" : "badge-virtual"}">
        ${e.type === "person" ? "In-Person" : "Virtual"}
      </span>
...
function renderAlumni(dataArr) {
  ...
  grid.innerHTML = data
    .map(
      (a, i) => `
    <div class="alumni-card" style="animation-delay: ${i * 0.08}s" id="alumni-${a.id}" data-category="${a.category}">
